...
public class TelaTabuleiroJogador extends JPanel {
	...
	private void desenhaTiros(Graphics g) {
		try {
			...
			for (Point pt : Controller.getTiros(Controller.getIdOponente())) {
				int valor = Controller.getJogador().getTabuleiro()
	...
	private void desenhaTabuleiro(Graphics g) {
		try {
			...
			for (int i = 1; i < 21; i++) {
				g.drawLine(i * 30, 0, i * 30, TAM_MAPA * DIM_QUADRADO); // linha
																		// horizontal
				g.drawLine(0, i * 30, TAM_MAPA * DIM_QUADRADO, i * 30); // linha
																		// vertical
			}
	...
	public void alteraOrientacaoNavio() {
		OrientacaoNavio orientacaoAntiga;
		try {
			orientacaoAntiga = Controller.getJogador().getNavio(idNavioAtual)
					.getOrientacao();
	...
	public void adicionarNavio() throws RemoteException {
			...
			try {
				...
				if (idNavioAtual == TOTAL_NAVIOS) {
					...
					if (Controller.getEstadoJogo() == Estado.POSICIONANDO_NAVIOS)
						principal
								.mostraEvento("Aguarde oponente posicionar todos os navios");
	...
	public void posicionarNavio() {
		try {
			Point posAntiga = Controller.getJogador().getNavio(idNavioAtual)
					.getPosicao();
			Controller.alteraPosicaoNavio(idNavioAtual, posicaoAtual);
